...
export type CompradorHistoricoResponse = components['schemas']['CompradorHistoricoResponse']
...
export async function buscarHistorico(id: number): Promise<CompradorHistoricoResponse> {
  const res = await api.get<CompradorHistoricoResponse>(`/vendedor/compradores/${id}/historico`)
  return res.data
}
